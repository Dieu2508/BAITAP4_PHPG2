<?php 
	class book{
		...
		var $price;
		var $year;
		function __construct ($id, $title, $author,$price, $year){
	    ...
	    static function search($search){
			...
			if (empty($search)) {
				...
				foreach ($file as $lsa){
					if (strlen(strstr(strtolower($search) , strtolower($lsa->id))) !=0 || strlen(strstr(strtolower($search) , strtolower($lsa->price)))!=0  || strlen(strstr(strtolower($lsa->title) , strtolower($search)))!=0 ||  strlen(strstr(strtolower($search) ,strtolower($lsa->author)))!=0||  strlen(strstr(strtolower($search) ,strtolower($lsa->year)))!=0) { ?>
						<?php echo  '<tr class="content" >
							<td>'. $lsa->id .'</td>
							<td>'. $lsa->title . '</td>
							<td>'. $lsa->author .'</td>
		...
		static function delete($id){
			...
			foreach ($dataNew as $value) {
				$myWrite.= $value->id. "#" . $value->title . "#" .   $value->author . "#" . $value->price . "#" .$value->year ;
			}
			fwrite($file, $myWrite);
			fclose($file);
		...
		static function edit($idedit,$editid,$giaedit,$titledit,$tacgiaedit,$yearedit){
			...
			foreach ($dataNews as $value) {
				$my_write.= $value->id . "#" .$value->title . "#" . $value->price . "#" . $value->author . "#" . $value->year ;
			}
